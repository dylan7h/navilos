...
extern __IO GicCput_t* GicCpu;
extern __IO GicDist_t* GicDist;
...
void Hal_interrupt_init(void)
{
    ...
    GicCpu->cpucontrol.bits.Enable = ENABLE;
    GicCpu->prioritymask.bits.Prioritymask = GIC_PRIORITY_MASK_NONE;
    GicDist->distributorctrl.bits.Enable = ENABLE;
...
void Hal_interrupt_enable(uint32_t interrupt_num)
{
    ...
    result = (interrupt_num < GIC_IRQ_START) || (interrupt_num > GIC_IRQ_END);
    if(result == 0)
    {
        bit_num = interrupt_num - GIC_IRQ_START;
